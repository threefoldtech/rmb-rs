...
use log;
use subxt::{config::Header, OnlineClient, PolkadotConfig, blocks::Block as SubxtBlock};
...
impl EventListenerOptions {
        ...
        C: Cache<Twin> + Clone,
    {
        ...
        let _ = self.registry.register(Box::new(EVENTS_RECONNECTING.clone()));
        let _ = self.registry.register(Box::new(EVENTS_RECONNECT_CYCLES.clone()));
        let _ = self.registry.register(Box::new(EVENTS_ERRORS.clone()));
        let _ = self.registry.register(Box::new(EVENTS_LAST_BLOCK_NUM.clone()));
        let _ = self.registry.register(Box::new(EVENTS_TWIN_STORED_TOTAL.clone()));
        let _ = self.registry.register(Box::new(EVENTS_TWIN_UPDATED_TOTAL.clone()));
    ...
    C: Cache<Twin> + Clone,
{
    pub async fn new(substrate_urls: Vec<String>, cache: C) -> Result<Self> {
        ...
        EventListenerOptions::new().build(substrate_urls, cache).await
    }
            ...
            Some(last) => {
                if head_num > last {
                    ...
                    if gap <= self.catchup_threshold {
                        log::info!("catching up {} blocks ({} -> {}) without flushing cache", gap, last, head_num);
                        ...
                        for n in (last + 1)..=head_num {
                            ...
                            count += 1;
                            if count % 50 == 0 { tokio::task::yield_now().await; }
                        }
                        Ok(())
                    } else {
                        log::warn!("gap {} exceeds threshold {}; flushing cache and jumping to head {}", gap, self.catchup_threshold, head_num);
    ...
    async fn handle_events_inner(&mut self) -> Result<()> {
        ...
        let mut blocks_sub = self.api.blocks().subscribe_best().await.map_err(|e| {
            e
        })?;
        ...
        while let Some(block) = match tokio::time::timeout(stall_timeout, blocks_sub.next()).await {
            Ok(v) => v,
            Err(_) => {
                return Err(anyhow::anyhow!("block subscription timed out after {:?}", stall_timeout));
            }
        } {
            let block = block?;
            self.process_block(block).await?;
    ...
    async fn process_block(&mut self, block: SubxtBlock<PolkadotConfig, OnlineClient<PolkadotConfig>>) -> Result<()> {
        ...
        for evt in events.iter() {
            ...
            if let Ok(Some(twin)) = evt.as_event::<tfchain::tfgrid_module::events::TwinStored>() {
                self.cache.set(twin.0.id, twin.0.into()).await.map_err(|e| {
